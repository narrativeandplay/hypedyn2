...
import org.narrativeandplay.hypedyn.ui.story.UiStory.UiStoryMetadata
...
class StoryPropertiesDialog(story: Narrative, ownerWindow: Window) extends Dialog[UiStoryMetadata] {
  ...
  dialogPane().scene().stylesheets += getClass.getResource("/org/narrativeandplay/hypedyn/ui/tab-pane-fix.css").toExternalForm
