...
\item Now we will make copies of this Anywhere node. Select the node in the Map View, and then choose the \textit{Copy} menu item under the \textit{Edit} menu. 
\item Now choose the \textit{Paste} menu item under the \textit{Edit} menu. A copy of the selected node will appear in the top left of the Map View. 
\item Repeat the previous step 5 times, so that in total you have 7 nodes named ``New Node''.

\item Now edit each of these nodes, and change the names and content to match the names and content as shown in Table \ref{table:tut3:node_content}.
