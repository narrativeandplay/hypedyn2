...
/**
  ...
  *
  * @param from The subtheme the link starts from
  * @param to The theme the link ends at
  * @param parentLinkGroup The group of links the link belongs to
  */
class ThemeSubthemeLink(val from: ViewerTheme,
           val to: ViewerTheme,
           private val parentLinkGroup: ThemeSubthemeLinkGroup) {
  ...
  private val linkLabel = new Label {
    ...
    maxHeight = ThemeSubthemeLink.LabelHeight
  ...
  def draw = {
    ...
    linkLabel.relocate(labelMidpoint.x - ThemeSubthemeLink.LabelWidth / 2, labelMidpoint.y - ThemeSubthemeLink.LabelHeight / 2)
