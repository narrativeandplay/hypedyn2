...
/**
  ...
  */
class ThematicLink(val from: ViewerNode,
           val to: ViewerNode,
           private val parentLinkGroup: ThematicLinkGroup) {
  private var closestBezierParam = -1d

  /**
    * The name of the link - maybe should bind to the name of the theme?
    */
  val name = "Thematic link"
  ...
  private val linkLabel = new Label {
    ...
    maxWidth = ThematicLink.LabelWidth
    maxHeight = ThematicLink.LabelHeight
  ...
  private val labelBackground = new Rectangle {
    ...
    height = ThematicLink.LabelHeight
    fill = ThematicLink.DefaultBackgroundColour
  ...
  def draw = {
    ...
    import ThematicLink.NameDisplayType._
    val label: Option[Label] = ThematicLink.NameDisplay match {
      case OnLinkAlways =>
        val labelMidpoint = pathCurve pointAt 0.5
        linkLabel.relocate(labelMidpoint.x - ThematicLink.LabelWidth / 2, labelMidpoint.y - ThematicLink.LabelHeight / 2)
        Some(linkLabel)
      case OnLinkOnClick =>
        if (selected()) {
          val labelMidpoint = pathCurve pointAt 0.5
